...
/6a3320a3523e1a77/                      Layer 5. OSINT — secret in git history.
/d178249adf841d68/                      Final. Auto-logging form → ntfy topic.
```

Companion repo: https://github.com/thebigjah/prowess-archive — public, planted with deliberately leaked secret in deleted-file git history.
...
- **Layer 4 → 5:** Layer 4 publishes a 188-bit RSA modulus n + e=65537 + ciphertext c. Solver factors n (sympy/msieve/factordb), computes d, decrypts c → plaintext is the 16-hex string `6a3320a3523e1a77` which IS the Layer 5 path. Primes are deterministic via sha256 seeds.
- **Layer 5 → final:** Layer 5 points to https://github.com/thebigjah/prowess-archive. Solver clones and uses git pickaxe (`git log -S`, `git log -p -- notes.txt`) to recover the deleted secret `KEYSTONE-7340-OPENS`, then `sha256("KEYSTONE-7340-OPENS" + "finish").slice(0,16)` = `d178249adf841d68` for the final path.
